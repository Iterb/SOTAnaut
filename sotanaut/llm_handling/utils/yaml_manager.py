...
class YAMLManager:
    ...
    def _load_yaml(self, category, key):
        ...
        if not yaml_path.exists() or yaml_path.suffix != '.yaml':
            raise YAMLFileNotFoundError(f"YAML file for '{key}' in category '{category.value}' not found.")
    ...
    def get(self, category, key):
        ...
        try:
            ...
            raise YAMLKeyNotFoundError(f"'{key}' not found in category '{category.value}'. Available keys: {available_keys}")
